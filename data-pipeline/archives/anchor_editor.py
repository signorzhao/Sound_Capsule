...
def create_lens():
    ...
    import re
    if not re.match(r'^[a-z][a-z0-9_]*$', lens):
        return jsonify({'success': False, 'message': '棱镜 ID 格式不正确'})
    
    # 检查是否已存在
    config = load_config()
    if lens in config:
        return jsonify({'success': False, 'message': f'棱镜 {lens} 已存在'})
    
    # 创建新棱镜配置
    new_lens = {
        'name': data.get('name', f'{lens.capitalize()} Lens'),
        'description': data.get('description', '自定义棱镜'),
        'lexicon_file': f'lexicon_{lens}.csv',
        'axes': data.get('axes', {
            'x_label': {'neg': 'X-', 'pos': 'X+'},
            'y_label': {'neg': 'Y-', 'pos': 'Y+'}
        }),
    ...
    # 保存配置
    config[lens] = new_lens
    save_config(config)
    
    return jsonify({
        'success': True, 
        'message': f'棱镜 {lens} 创建成功！词库文件: {new_lens["lexicon_file"]}'
    })
