...
开发版前端默认请求 **本机** `http://localhost:5002`。若 API 跑在**另一台机器**（如 Mac 或局域网服务器），需要配置 API 基地址：

1. 打开应用 → **用户菜单** → **设置**（或首次启动时的设置向导）。
2. 在 **「API 服务器」** 中填写 **API 服务器地址（可选）**，例如：
   - `http://192.168.1.100:5002`（局域网内另一台机器的 API）
   - `http://本机IP:5002`（本机 API 但用 IP 访问时）
3. 保存后，**刷新页面或重启应用**，后续请求会发往该地址。

不填则默认使用 `http://localhost:5002`（本机 5002 端口）。
...
### Q: Windows 开发版为什么连不到「本地部署的服务器」？

**A**: 前端原先写死 `http://localhost:5002`，只会连**本机**。若 API 跑在另一台机器（如 Mac 或内网服务器），需要：

1. 在应用 **设置** 里填写 **API 服务器地址**，如 `http://192.168.x.x:5002`。
2. 保存后刷新或重启应用。

详见上文「连接本地部署的 API 服务器」。
