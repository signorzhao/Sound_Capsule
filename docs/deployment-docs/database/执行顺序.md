...
启用胶囊语义搜索时，在项目 `data-pipeline/database/supabase_migrations/` 下按顺序执行：

| 顺序 | 文件名 | 说明 |
|------|--------|------|
| 8 | **008_profiles_add_language.sql** | 用户语言偏好（表不存在时会先创建 `profiles` 再加列，可直接执行） |
| 9 | **009_cloud_capsules_embedding_pgvector.sql** | pgvector + embedding 列 + semantic_search_capsules |
| 10 | **010_tag_level_embedding.sql** | 标签级 embedding + semantic_search_capsules_tag_level |

详见 [SUPABASE_PRIVATE_DEPLOY.md](../../SUPABASE_PRIVATE_DEPLOY.md) 第三节、[应用更新记录.md](../../应用更新记录.md)。
...
- **008**：001～007 未创建 `profiles` 表，若直接执行旧版 008 会报错 `relation "profiles" does not exist`。当前 008 已改为「无表则先建表再加列」，本地/自建按顺序执行即可，无需先手动建表。
