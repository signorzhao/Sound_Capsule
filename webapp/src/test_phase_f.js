...
if (fs.existsSync(configRsPath)) {
    ...
    const checks = [
        ...
        ['get_app_config command', /#\[tauri::command\]\s*pub async fn get_app_config/],
        ['save_app_config command', /#\[tauri::command\]\s*pub async fn save_app_config/],
        ['reset_app_config command', /#\[tauri::command\]\s*pub async fn reset_app_config/],
    ];
...
if (fs.existsSync(initialSetupPath)) {
    ...
    const checks = [
        ...
        ['文件选择对话框', /from '@tauri-apps\/plugin-dialog'/,
        ['配置 API 调用', /from.*configApi/,
    ];
